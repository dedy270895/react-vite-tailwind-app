...
const BudgetCard = ({ budget, onEdit, onDelete }) => {
  ...
  const [editAmount, setEditAmount] = useState(budget?.allocated);
  ...
  const handleSaveEdit = () => {
    onEdit(budget?.id, parseFloat(editAmount));
    setIsEditing(false);
  };

  const handleCancelEdit = () => {
    setEditAmount(budget?.allocated);
    setIsEditing(false);
  };
    ...
    <div className="bg-card border border-border rounded-lg p-4 md:p-6 shadow-soft">
      ...
      <div className="flex items-center justify-between mb-4">
        <div className="flex items-center space-x-3">
          <div className={`w-10 h-10 rounded-lg ${budget?.color} flex items-center justify-center`}>
      ...
      <div className="mb-4">
        <div className="flex items-center justify-between text-sm text-muted-foreground mb-2">
          <span>Spent: ${budget?.spent?.toLocaleString('en-US', { minimumFractionDigits: 2 })}</span>
          <span>Remaining: ${remainingAmount?.toLocaleString('en-US', { minimumFractionDigits: 2 })}</span>
        </div>
      ...
      {spentPercentage >= 100 && (
        <div className="flex items-center space-x-2 p-3 bg-error/10 border border-error/20 rounded-lg">
          ...
          <span className="text-sm text-error font-medium">
            Budget exceeded by ${(budget?.spent - budget?.allocated)?.toLocaleString('en-US', { minimumFractionDigits: 2 })}
          </span>
        </div>
      )}
