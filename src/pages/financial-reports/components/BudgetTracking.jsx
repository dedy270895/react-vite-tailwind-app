...
const BudgetTracking = ({ selectedPeriod }) => {
  ...
  const getStatusColor = (status) => {
    switch (status) {
      ...
      case 'over-budget': return 'text-error';
  ...
  const getStatusBg = (status) => {
    switch (status) {
      ...
      case 'over-budget': return 'bg-error';
    ...
    if (active && payload && payload?.length) {
      ...
      return (
        <div className="bg-card border border-border rounded-lg p-3 shadow-elevated">
          ...
          <p className="text-sm text-primary">Budget: ${data?.budget?.toLocaleString()}</p>
          <p className="text-sm text-error">Spent: ${data?.spent?.toLocaleString()}</p>
          <p className="text-sm text-muted-foreground">
            {data?.remaining >= 0 ? 'Remaining' : 'Over'}: ${Math.abs(data?.remaining)?.toLocaleString()}
          </p>
        </div>
      );
    }
            ...
            <div key={index} className="bg-muted/30 rounded-lg p-4">
              ...
              >
                <div className="flex items-center space-x-3">
                  ...
                  <div>
                    <h5 className="font-medium text-foreground">{budget?.category}</h5>
                    <p className="text-sm text-muted-foreground">
              ...
              <div className="mt-3">
                <div className="w-full bg-muted rounded-full h-3">
                  <div 
                    className={`h-3 rounded-full transition-all duration-300 ${getStatusBg(budget?.status)}`}
                    style={{ 
                ...
                <div className="mt-4 pt-4 border-t border-border">
                  <div className="grid grid-cols-2 md:grid-cols-3 gap-4 text-sm">
                    <div>
                      ...
                      <p className="font-medium text-foreground">${budget?.dailyAverage}</p>
                    </div>
                    <div>
                      <span className="text-muted-foreground">Projected Spend</span>
                      <p className="font-medium text-foreground">${budget?.projectedSpend?.toLocaleString()}</p>
                    </div>
                    <div>
